...
**Обрезка заголовка:**
- Резервируется место под индикатором и рамкой
- Заголовок ANSI-aware усекается по ширине терминальных ячеек
- Широкие Unicode-графемы не разрезаются посередине

**Отступы:**
- Вычисляются по отображаемой ширине заголовка в ячейках
- Если остаточная ширина отрицательная — используется 0
...
- Заголовок усекается по ширине ячеек; байтовая длина Unicode-текста не определяет геометрию
