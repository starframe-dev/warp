...
Настраивает символ сворачивания на границе вертикального разделения, содержащего панель.
...
- `parent` — панель, определяющая разделение
- `collapseRow` — строка (0-индексация), где на границе вместо «│» отображается «<»
- `onCollapse` — callback, вызываемый по клику на символ; возвращает команду перерисовки
...
Переключает состояние сворачивания разделения, содержащего указанную панель. Используется, когда панель сама обрабатывает раскрытие и нужно синхронизировать состояние узла warp.
...
- `parent` — панель внутри разделения
