...
- Автоматическая нормализация координат и размеров при изменении viewport, включая создание до получения первого размера
...
- Заголовок обрезается ANSI-aware по ширине доступных терминальных ячеек
- Контент отрисовывается с отступом по одной ячейке слева и справа
- Используются стили `floatBgStyle`, `floatBorderStyle`, `floatTitleStyle`, `floatCloseStyle`
...
**Реализация**:
- Использует ANSI-aware удаление escape-последовательностей
- Не изменяет текстовые байты, оставшиеся после удаления последовательностей
...
- `floatCloseStyle` — стиль кнопки закрытия (×)
