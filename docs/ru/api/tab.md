...
- **Очистка фокуса:** `CloseFloat` вызывает `Blur` и очищает target, если
  удаляет текущую сфокусированную плавающую панель. После клика снаружи
  обработчик может сфокусировать панель под указателем.
