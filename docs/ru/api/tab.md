...
- **Жизненный цикл ресурсов:** самостоятельный `Tab` владеет корневым
  деревом и float-панелями; вкладка внутри `TabGroup`/`Warp` использует
  внешнего владельца. При удалении последней ссылки на панель её
  необязательный `Unmount` вызывается после detach. Reparenting и
  визуальная видимость не вызывают `Unmount`.
